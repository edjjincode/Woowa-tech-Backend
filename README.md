...
- ✅ 고객들한테 식당 예상 방문 날짜를 받는다
    - 방문할 날짜는 1 이상 31 이하의 숫자로만 입력받아 주세요.
    - 1 이상 31 이하의 숫자가 아닌 경우, "[ERROR] 유효하지 않은 날짜입니다. 다시 입력해 주세요."라는 에러 메시지를 보여 주세요.
    - 모든 에러 메시지는 "[ERROR]"로 시작하도록 작성해 주세요.
- ✅ 고객들한테 주문할 메뉴와 개수를 입력받는다
    - 고객이 메뉴판에 없는 메뉴를 입력하는 경우, "[ERROR] 유효하지 않은 주문입니다. 다시 입력해 주세요."라는 에러 메시지를 보여 주세요.
    - 메뉴의 개수는 1 이상의 숫자만 입력되도록 해주세요. 이외의 입력값은 "[ERROR] 유효하지 않은 주문입니다. 다시 입력해 주세요."라는 에러 메시지를 보여 주세요.
    - 메뉴 형식이 예시와 다른 경우, "[ERROR] 유효하지 않은 주문입니다. 다시 입력해 주세요."라는 에러 메시지를 보여 주세요.
    - 중복 메뉴를 입력한 경우(e.g. 시저샐러드-1,시저샐러드-1), "[ERROR] 유효하지 않은 주문입니다. 다시 입력해 주세요."라는 에러 메시지를 보여 주세요.
    - 모든 에러 메시지는 "[ERROR]"로 시작하도록 작성해 주세요.
- ✅ 고객들이 방문 날짜 및 메뉴를 입력하면 이벤트 플래너가 주문 메뉴, 할인 전 총주문 금액, 증정 메유, 혜택 내역, 총혜택 금액, 할인 후 예상 결제 금액 12월 이벤트 배지 내용을 보여주기를 기대한다
- ✅ 크리스마스 디데이 할인이 적용된다
- ✅ 평일 할인이 적용된다
- ✅ 주말 할인이 적용된다
- ✅ 특별 할인이 적용된다
- ✅ 증정 상품을 주는 이벤트가 진행된다
- ✅ 총 혜택 금액을 구한다(총혜택 금액 = 할인 금액의 합계 + 증정 메뉴의 가격)
    - 총혜택 금액 = 할인 금액의 합계 + 증정 메뉴의 가격
    - 고객에게 적용된 이벤트 내역만 보여 주세요.
    - 적용된 이벤트가 하나도 없다면 혜택 내역 "없음"으로 보여 주세요.
    - 혜택 내역에 여러 개의 이벤트가 적용된 경우, 출력 순서는 자유롭게 출력해주세요.
- ✅ 총혜택 금액에 따라 이벤트 배지의 이름을 출력한다
- ✅ 증정 메뉴
- ✅ 혜택 내역
- ✅ 이벤트 배지
...
1. 고객들한테 식당 예상 방문 날짜를 입력받는다
2. 고객들한테 주문할 메뉴와 개수를 입력받는다
3. 고객의 방문 날짜 및 주문한 메뉴 종류 및 개수를 구한 후 해당 값을 이벤트 플래너에 저장한다
4. 고객이 방문한 날짜를 기준으로 할인율을 게산한다
- 할인 후 예상 결제 금액 = 할인 전 총주문 금액 - 할인 금액
5. 고객이 주문한 메뉴 및 개수를 기준으로 총 금액을 게산한다. 총 주문 금액을 통해 증정품 수여 여부를 결정한다. 이때 총 주문 금액은 증정품 가격 또한 포함된다
6. 주문한 메뉴를 출력한다
7. 할인전 총 주문금액을 출력한다
8. 증정 메뉴 여부를 출력한다
- 증정 이벤트에 해당하지 않는 경우, 증정 메뉴 "없음"으로 보여 주세요.
9. 혜택 여부를 출력한다
10. 할인 후 예상 결제 금액 출력한다
11. 12월 달 예상 금액을 출력한다

## 깃허브 커밋 순서

1. READme 파일을 작성한다
2. 전체적인 코드 구성을 한다
3. 고객들한테 방문 날짜를 입력받는 코드를 작성한다
4. 고객들한테 주문할 메뉴와 개수를 입력받는 코드를 작성한다
5. 고객들한테 입력받은 값을 메뉴와 개수로 나눠서 따로 User class에 저장한다
6. 고객들한테 입력받은 값이 주말인지 평일인지 구분하는 클래스를 작성한다
7. 고객들한테 입력받은 날짜를 통해 할인율을 게산한다
 - 1. 날짜를 통해 크리스마스 할인율을 계산한다
...
10. 음료만 주문 시 주문할 수 없도록 한다.
11. 메뉴는 총 20개만 주문할 수 있도록 한다.
12. 출력 값들을 출력한다
  - 주문 메뉴
  - 할인 전 총주문 금액
  - 증정 메뉴
